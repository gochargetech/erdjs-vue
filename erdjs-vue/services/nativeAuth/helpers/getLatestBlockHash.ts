...
import { BLOCKS_ENDPOINT } from 'erdjs-vue/apiCalls/endpoints';
...
const getLatestBlockHashFromServer = retryMultipleTimes(
  async (
    ...
    const { data } = await axios.get<Array<LatestBlockHashType>>(
      `${apiUrl}/${BLOCKS_ENDPOINT}?from=${getBlockFromPosition}&size=1&fields=hash,timestamp${blockHashShard ? '&shard=' + blockHashShard : ''
      }`
    );
