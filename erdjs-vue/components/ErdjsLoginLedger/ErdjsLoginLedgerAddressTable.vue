                                ...
                                <template v-if="!getIsLoading && getAccounts">
                                    ...
                                    <ul class="d-flex flex-column justify-content-start align-items-start">
                                        <li v-for="(address, aIndex) in getAccounts" v-bind:key="`ledger-account-${aIndex}`"
                                            ...
                                            class="d-flex flex-row justfy-content-start mb-2 ledger-account mw-100 overflow-auto">
                                            <input class="form-check-input me-2" type="radio" :name="`ledgerRadio${address}`" :id="`ledgerRadio${address}`">
                                            <span>({{ getAddressIndex(aIndex) }})</span>
                                            <span>{{ address }}</span>
                                        </li>
                                    </ul>
